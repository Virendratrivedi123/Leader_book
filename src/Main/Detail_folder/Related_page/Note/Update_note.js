...
import {
  ...
  View,
  StyleSheet,
  TouchableOpacity,TextInput,Image,Modal,Pressable
} from "react-native";
...
import { Add_Note, Update_lead_Note } from "../../../../Services";
  ...
  const postdata = async () => {
    try {
      ...
      const data = {
        email: d.email,
        password: d.password,
        id: route?.params?.lead_id,
       note:note,
       note_id:route?.params?.user?.id
      };
      Update_lead_Note(data).then((response) => {
          ...
          >
            <View style={styles.centeredView_box}>
              <View style={styles.modalView_box}>
                ...
                <Text style={styles.textStyle2_box}>Note has been updated successfully.</Text>
                <View
                  style={{
                    height: 1,
                ...
                <Pressable
                  style={{  }}
                  onPress={() => {
                    setModalVisible5(!modalVisible5),navigation.pop(1)
                  }}
                >
...
const styles = StyleSheet.create({
  ...
  input: {
    ...
    color: "#6c6c6c",
    fontSize: wp("4.8%"),fontFamily: "Inter-Black2"
  },
  button: {
    height: height * 0.085,
    width: width * 0.9,
